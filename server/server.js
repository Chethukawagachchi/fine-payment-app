...
app.post("/api/login", async (req, res) => {
  ...
  try {
    const result = await pool.query("SELECT * FROM citizens WHERE email = $1 AND license_id = $2", [
      email,
      licenseId,
    ]);
...
app.post("/api/fines", async (req, res) => {
  const { officerId, licenseId, fineName, fineType, fineAmount, firstName, lastName, email, phoneNumber } = req.body;

  try {
    // Ensure the citizen exists in the citizens table
    const citizenResult = await pool.query("SELECT * FROM citizens WHERE license_id = $1", [licenseId]);
...
visit our website - https://fine-payment-react.vercel.app/
...
Fine.lk`,
    ...
    res.status(201).json({ message: "Fine issued successfully and email sent!" });
  } catch (err) {
...
app.get("/api/officerfines", async (req, res) => {
  ...
  try {
    const result = await pool.query("SELECT * FROM officerfines WHERE license_id = $1", [licenseId]);
    res.status(200).json(result.rows);
  } catch (err) {
